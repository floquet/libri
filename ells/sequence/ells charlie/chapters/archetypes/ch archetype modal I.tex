...
The first plot is the solution curve plotted against the measurements as shown in figure \ref{fig:bevington residuals}. The residual errors are shown as red segments which are the actual components of the residual error vector in $\cmplx{9}$.
...
\begin{figure}
\centering
    \begin{overpic}[ scale = 0.75 ]{\pathgraphics "bevington plus"/"n merit bullseye"}
        \put(25,103){$M(a) = \sum_{k=1}^{m}\paren{T_{k} - a_{0} - a_{1} x}^{2}$}
    	\put(50,-3){$a_{0}$}
    	\put(-5,52){$a_{1}$}
    \end{overpic}
   \label{fig:bevington residuals}
...
\begin{figure}
\centering
    \begin{overpic}[ scale = 0.75 ]{\pathgraphics "bevington plus"/"n merit bullseye lines"}
        \put(25,103){$M(a) = \sum_{k=1}^{m}\paren{T_{k} - a_{0} - a_{1} x}^{2}$}
        %
    	\put(50,-3){$a_{0}$}
    	\put(-5,52){$a_{1}$}
  ...
  \begin{table}[htbp]  %  T A B L E
    ...
    \begin{center}
      \begin{tabular}{cccr@{.}lr@{.}lr@{.}l}
        %
        &&& \multicolumn{2}{c}{ } & \multicolumn{2}{c}{(actual)} & \multicolumn{2}{c}{(ideal)} \\
        %
