...
Success in manipulating the \asvd \ includes success in manipulating the $\sig{}$ matrices.
The breakdown of block diagrams.
  \begin{equation*}   %  =   =   =   =   =
    \begin{split}
  ...
  \begin{equation*}   %  =   =   =   =   =
    \begin{split}
       \sig{} = 
         \mat{c|c}{\ess{}_{\rho \times \rho} & \zero_{\rho \times n-\rho} \\ \zero_{m-\rho \times \rho} & \zero_{m-\rho \times n-\rho} } \\
    \end{split}
   %\label{eq:}
  \end{equation*}
...
Notice the interchange of the indices $m$ and $n$ in the transpose:
  \begin{equation*}   %  =   =   =   =   =
    \begin{split}
       \sig{T} = 
         \mat{c|c}{\ess{}_{\rho \times \rho} & \zero_{\rho \times m-\rho} \\ \zero_{n-\rho \times \rho} & \zero_{n-\rho \times m-\rho} }.
    \end{split}
   %\label{eq:}
  \end{equation*}
...
The pseudoinverse matrix has the same dimension as the parent matrix:
  ...
  \begin{equation*}   %  =   =   =   =   =
    %\begin{split}
      ...
      \sig{\sym} = 
        \mat{cc}{ 1 / \sigma_{1} & 0 \\ 0 & 1 / \sigma_{2} \\ 0 & 0 \\ 0 & 0 }, \quad
      \sig{T} = 
  ...
  \begin{equation*}   %  =   =   =   =   =
    \begin{split}
      \sig{} \sig{\sym} &= \II{4,2} = \mat{cc}{\I{2} & \zero \\ \zero &  \zero} = \mat{cc}{1 & 0 \\ 0 & 1 \\ 0 & 0 \\ 0 & 0 }, \\
      \sig{\sym} \sig{} &= \II{2,2} = \I{2} = \idtwo.
    \end{split}
   %\label{eq:}
  \end{equation*}
...
\begin{table}[htbp]  % + + + + T A B L E
    ...
    \begin{center}
        \begin{tabular}{llclclcl}
            ...
            &\multicolumn{4}{l}{Range space} & \multicolumn{3}{l}{Null space} \\\hline
            %
            domain & $\pras$   & = & $\bvr{*} \spa \bvr{}$ && $\pna$   & = & $\I{n} - \bvr{*} \spa \bvr{}$\\
            %
            codomain & $\pra$  & = & $\bur{} \spb \bur{*}$ && $\pnas$  & = & $\I{m} - \bur{*} \spb \bur{}$\\
            %
        \end{tabular}
    \end{center}
    %\label{default}
