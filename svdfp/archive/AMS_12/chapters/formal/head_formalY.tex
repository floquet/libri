...
\begin{equation}
  ...
  \end{split}
\end{equation}
The solution comes from the first element of the vector equality
\begin{equation}
  x_{1} - x_{2} = \frac{1}{3}.
\label{eq:formal:2}
\end{equation}
This is the homogenous (null space) solution\index{homogenous solution} which goes through the particular solution. To recover the particular solution\index{particular solution}, find where the range of the matrix $\A{}$ crosses this line. From the domain basis matrix $\X{}$ we see that the range is the line
\begin{equation}
x_{2}=-x_{1}.
\label{eq:formal:1}
\end{equation}
